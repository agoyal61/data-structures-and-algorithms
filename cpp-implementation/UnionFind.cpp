...
class UnionFind {
private:
    vector<int> parent, rank, wt;
public:
    UnionFind(int n){
        parent=vector<int>(n);
        rank=vector<int>(n,0);
        wt=vector<int>(n,131071);
        for(int i = 0; i < n; i++) parent[i] = i;
    }

    // Member Initializer List // another ways to initialize the member variables
    // UnionFind(int n) : parent(vector<int>(n)), rank(vector<int>(n, 0)), wt(vector<int>(n, 131071)) {
    //     for(int i = 0; i < n; i++) {
    //         parent[i] = i;
    //     }
    // }
